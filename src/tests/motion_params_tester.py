...
def get_event_tracker():
    class EventTracker:
        ...
        def track_frame(self, frame_obj):
            ...
            if frame_obj.motion:
                cv2.putText(frame_obj.frame, "X".format(frame_obj.index, frame_obj.motion),
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (64, 64, 255), 2)
...
if __name__ == "__main__":
    ...
    for history in [128]:
        for threshold in [128]:
            for bg_ration in [0.2]:
                for shadow in [0.2, 0.5, 0.8]:
                    start = time.time()
